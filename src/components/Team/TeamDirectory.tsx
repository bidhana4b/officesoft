...
import { Search, Mail, Phone, MoreHorizontal, Plus, Edit } from 'lucide-react';
...
const TeamMemberCard: React.FC<TeamMemberCardProps> = ({ member, index, onEditMember }) => {
        ...
        <div className="relative" ref={menuRef}>
          ...
          <AnimatePresence>
            {isMenuOpen && (
              ...
              >
                <button onClick={() => { onEditMember(member); setIsMenuOpen(false); }} className="w-full text-left flex items-center space-x-2 px-3 py-2 rounded-lg hover:bg-sand-100 dark:hover:bg-dark-600 text-charcoal-700 dark:text-gray-200">
                  <Edit size={16} /><span>Edit Member</span>
                </button>
              </motion.div>
            )}
          </AnimatePresence>
...
export const TeamDirectory: React.FC<TeamDirectoryProps> = ({ teamMembers, onAddMember, onEditMember }) => {
  ...
  const departments = ['all', ...Array.from(new Set(safeTeamMembers.map(m => m.department)))];
